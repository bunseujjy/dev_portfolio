...
import { techology_code } from "@/lib/terminalcode";
import { IoMdArrowDown } from "react-icons/io";
import TechnologySidebar from "../component/TechnologySidebar";

const Technology = () => {
  const [tabs, setTabs] = useState("frontend.json");
  const [allTabs, setAllTabs] = useState(["frontend.json"]);
          ...
          <div className="flex flex-col lg:flex-row h-full overflow-hidden">
            ...
            >
              <div className="flex items-center justify-between text-sm border border-slate-700 rounded-md p-2">
                <p className="text-xs">skill / tech-stack / {tabs}</p>
                <IoMdArrowDown
            ...
            <div className="flex flex-col w-full h-full">
              {icons.skill.find((i) => i.title === tabs) && (
                <ScrollArea className="h-full p-4">
                  <TechnologyContent tabs={tabs} />
                </ScrollArea>
              )}
            </div>
          </div>
